...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        ], "Installing image processing dependencies", website_dir):
            self.stdout.write(
                self.style.ERROR('Failed to install dependencies')
            )
            return
        ...
        if not self.run_command([
            ...
            "--quality-photos", str(options['quality_photos']),
            "--quality-graphics", str(options['quality_graphics'])
        ], "Converting images to WebP", website_dir):
            self.stdout.write(
                self.style.ERROR('Failed to convert images to WebP')
            )
            return
        ...
        if not self.run_command([
            ...
            "--minify-svg",
            "--input-dir", str(static_dir)
        ], "Minifying SVG files", website_dir):
            self.stdout.write(
                self.style.ERROR('Failed to minify SVG files')
            )
            return
        ...
        ], "Updating template references", website_dir):
            self.stdout.write(
                self.style.ERROR('Failed to update templates')
            )
            return
